...
{
	if(GetPixelSoftImage(h, x, y, &Color_R, &Color_G, &Color_B, &Color_A) != 0) // ? Ž¸”s
	{
		error();
	}
...
void SISavePixel(int h, int x, int y)
{
	...
	if(DrawPixelSoftImage(h, x, y, Color_R, Color_G, Color_B, Color_A) != 0) // ? Ž¸”s
	{
		error();
	}
}
...
int Pic_Load(int resno, autoList<uchar> *image)
{
	...
	if(si_filter1 || si_filter2)
	{
		...
		if(GetSoftImageSize(h, &x_size, &y_size) != 0) // ? Ž¸”s
		{
			error();
		}
		...
		errorCase(grph_h == -1); // ? Ž¸”s

		if(DeleteSoftImage(h) == -1) // ? Ž¸”s
		{
			error();
		}
		h = grph_h;
...
singleton(
	SoftImageFilterList,
	autoList<void (*)(int, int, int)>,
	makeList(P_MAX, (void (*)(int, int, int))NULL)
	)
singleton(
	GraphicHandleFilterList,
	autoList<void (*)(int)>,
	makeList(P_MAX, (void (*)(int))NULL)
	)
singleton(
	EnableTransList,
	autoList<int>,
...
int Pic(int resno)
{
	...
	/*
		‚±‚±‚ÅÝ’è‚³‚ê‚½’l‚ðŽŸ‚Ì•`‰æŽž‚ÉŽQÆ‚·‚éB
		‚Ì‚Å DrawPic(x, y, Pic(P_*)) ‚Ì‚æ‚¤‚ÉA•`‰æŒnŠÖ”‚Ìˆø”‚É’¼Ú Pic() ‚ð‹Lq‚µ‚ÄA
		Pic() ‚ÌŒÄ‚Ño‚µ‚Æ•`‰æ‚ÌŠÔ‚É•Ê‚Ì Pic() ‚ª“ü‚é—]’n‚Ì–³‚¢‹Lq‚ðSŠ|‚¯‚é‚±‚ÆB
	*/
...
void SetAlpha(double a) // a: 0.0 - 1.0 == “§–¾ - •s“§–¾
{
	SetBlend(DX_BLENDMODE_ALPHA, a);
}
void SetBlendAdd(double a) // a: 0.0 - 1.0 == •`‰æ‚µ‚È‚¢ - ^‚Á”’?
{
	SetBlend(DX_BLENDMODE_ADD, a);
}
void SetBlendInv(double a) // a: •sŽg—p?
{
...
void SetBright(double r, double g, double b) // (r, g, b): 0.0 - 1.0 == • - FÊ‚ ‚è
{
	m_range(r, 0.0, 1.0);
	m_range(g, 0.0, 1.0);
	m_range(b, 0.0, 1.0);
...
void DrawRotate(double rot) // rot: ŽžŒv‰ñ‚è, 0.0 == 0“x, PI == 180“x
{
	double w;
...
void DrawEnd(void)
{
	SetDrawMode(DX_DRAWMODE_BILINEAR); // ƒMƒƒƒU‘Îô
	...
	SetDrawMode(DX_DRAWMODE_NEAREST); // ƒfƒtƒH‚É–ß‚·B
}
